...
import pandas as pd ### TJM added this line
# import logging
...
def create_buffer_geopandas(inGDF,
                            #buffer_distance_meters=2,   ###TJM commented this line
                            buffer_cap_style=2, dissolve_by='class',   ###TJM buffer_cap_style = 1 -> 2
    ...
    buffer_distance_meters = inGDF['buff_dist']
    ...
    if projectToUTM:
        tmpGDF = osmnx_funcs.project_gdf(inGDF) ###TJM: deleted ,inGDF.crs
    else:
        tmpGDF = inGDF
...
def get_road_buffer(geoJson, im_vis_file, output_raster,
                     #buffer_meters=2, ### TJM deleted buffer_meters=2
                     burnValue=1,
                     # max_mask_val=1,
                     buffer_cap_style=2,   ### TJM changed buffer_cap_style=1 -> 2
                     useSpacenetLabels=True, ###TJM: False -> True 
    ...
    if useSpacenetLabels:
        inGDF = inGDF_raw
        inGDF['buff_dist'] = pd.to_numeric(inGDF['lane_number']) * 3   ### TJM added this line
    ...
    gdf_buffer = create_buffer_geopandas(inGDF,
                                         #buffer_distance_meters=inGDF['buff_dist'],   ### TJM changed buffer_meters=2 -> inGDF['buff_dist']
    ...
    if plot_file:
        ...
        ax2.axis('off')
        #ax2.set_title('Roads Mask (' + str(np.round(buffer_meters))   ### TJM commented out
                      #+ ' meter buffer)', fontsize=fontsize)   ### TJM commented out
